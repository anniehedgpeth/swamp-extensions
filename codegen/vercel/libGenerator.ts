// Generates _lib/vercel.ts — shared HTTP client and CRUD helpers for Vercel models

import { generateCopyrightHeader } from "../shared/licenseGenerator.ts";

/**
 * Generates the shared helper file that all Vercel extension models import.
 * Contains auth, HTTP client, rate-limit retry, pagination helpers, and CRUD operations.
 */
export function generateVercelLibFile(): string {
...
async function request(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (!resp.ok && resp.status !== 404) {
      ...
      let msg = \`Vercel API error: \${method} \${path} returned \${resp.status}: \${text}\`;
      try {
        const parsed = JSON.parse(text);
        if (parsed?.error?.message) {
          msg = \`Vercel API error: \${method} \${path} returned \${resp.status}: \${parsed.error.code}: \${parsed.error.message}\`;
        }
...
export async function read(
  ...
  const resp = await request("GET", \`\${endpoint}/\${encodeURIComponent(id)}\`, undefined, auth, team);
  if (resp.status === 404) {
    const text = await resp.text();
    throw new Error(
...
export async function update(
  ...
  const resp = await request(method, \`\${endpoint}/\${encodeURIComponent(id)}\`, body, auth, team);
  if (resp.status === 404) {
    const text = await resp.text();
    throw new Error(
...
export async function remove(
  ...
  const resp = await request("DELETE", \`\${endpoint}/\${encodeURIComponent(id)}\`, undefined, auth, team);
  if (resp.status === 404) {
    await resp.text();
...
export async function listAll(
  ...
  if (style === "cursor") {
    const params: Record<string, string> = { ...(queryParams ?? {}), limit: "100" };
    ...
    while (page < maxPages) {
      ...
      const resp = await request("GET", endpoint, undefined, auth, team, params);
    ...
    const resp = await request("GET", endpoint, undefined, auth, team, queryParams);
    const data = await resp.json() as Record<string, unknown>;
    const items = extractArrayFromResponse(data, endpoint);
    if (items) {
      for (const item of items) {
...
function extractArrayFromResponse(
  ...
  if (lastSegment && !lastSegment.startsWith("{") && Array.isArray(obj[lastSegment])) {
