// Generates _lib/gcp.ts — shared HTTP client and CRUD helpers for GCP models
//
// GCP uses service account credentials for auth, Discovery Document path
 ...
 */
async function getCredentials(explicit?: ExplicitGcpCredentials): Promise<GcpCredentials> {
  const quotaProjectId = explicit?.quotaProject || Deno.env.get("GOOGLE_CLOUD_QUOTA_PROJECT")?.trim() || undefined;

  // Explicit credentials from global args take highest precedence (vault expressions).
  if (explicit?.accessToken) {
    const projectId = explicit.project ?? Deno.env.get("GCP_PROJECT")?.trim() ?? Deno.env.get("GOOGLE_CLOUD_PROJECT")?.trim() ?? "";
  ...
  if (explicit?.credentialsJson) {
    ...
    if (explicit.project) {
      return { projectId: explicit.project, accessToken: creds.accessToken, quotaProjectId };
    }
  ...
  if (directToken) {
    const projectId = explicit?.project ?? Deno.env.get("GCP_PROJECT")?.trim() ?? Deno.env.get("GOOGLE_CLOUD_PROJECT")?.trim();
    return { projectId: projectId ?? "", accessToken: directToken, quotaProjectId };
  }

  if (cachedCredentials && (Date.now() - cachedAt) < TOKEN_TTL_MS) {
    if (explicit?.project) {
      return { projectId: explicit.project, accessToken: cachedCredentials.accessToken, quotaProjectId };
    }
...
async function activateServiceAccountFromJson(
  ...
  const projectOverride = Deno.env.get("GCP_PROJECT")?.trim() || Deno.env.get("GOOGLE_CLOUD_PROJECT")?.trim();
  ...
  if (config.parameters) {
    for (const [name, paramDef] of Object.entries(config.parameters)) {
      if (paramDef.location === "query" && params[name] !== undefined) {
        queryParams.push(\`\${encodeURIComponent(name)}=\${encodeURIComponent(params[name])}\`);
      }
    }
  }
...
export async function request(
  ...
  const safeBody = (upperMethod === "GET" || upperMethod === "HEAD") ? undefined : body;
 ...
 */
async function pollOperation(operationUrl: string, credentials?: ExplicitGcpCredentials): Promise<any> {
  ...
  throw new Error(\`Operation timed out after \${maxAttempts} attempts: \${operationUrl}\`);
}
...
function isLongRunningOperation(response: any): boolean {
  ...
  if (response.kind && typeof response.kind === "string" && response.kind.includes("#operation")) return true;
  // Generic LRO pattern: name contains "operations/" (v3 APIs may omit "done" initially)
  if ("name" in response && typeof response.name === "string" && response.name.includes("operations/")) return true;
  // GKE / Container API pattern: has operationType + status + name starting with "operation-"
  if (response.operationType && typeof response.operationType === "string" && response.status && typeof response.name === "string" && response.name.startsWith("operation")) return true;
  return false;
}
 ...
 */
function getOperationUrl(baseUrl: string, response: any, requestPath?: string): string {
  ...
  if (response.name && typeof response.name === "string") {
    ...
    let versionPrefix = "";
    if (requestPath) {
      const versionMatch = requestPath.match(/^(v[0-9]+(?:p[0-9]+)?(?:(?:alpha|beta)[0-9]*)?)\\//);
      if (versionMatch) {
...
export async function createResource(
  ...
  if (resp.status === 409 && idempotency) {
    ...
    const existing = await tryReadViaList(
      ...
      idempotency.matchField, idempotency.matchValue, credentials,
    );
  ...
  if (isLongRunningOperation(result)) {
    ...
    if (operation.response && typeof operation.response === "object" && operation.response.name && !params["name"]) {
    ...
    if (readConfig) {
      try {
        ...
        if (readResp.ok) {
          ...
          const targetResp = await request("GET", appendFieldsParam(operation.targetLink), undefined, credentials);
          if (targetResp.ok) {
...
export async function tryReadResource(
  ...
  if (resp.status === 403) {
    const text = await resp.text();
    throw new Error(\`Access denied (\${resp.status}): \${text}. Check IAM permissions.\`);
  }
 ...
 */
export async function getProjectId(credentials?: ExplicitGcpCredentials): Promise<string> {
  const creds = await getCredentials(credentials);
