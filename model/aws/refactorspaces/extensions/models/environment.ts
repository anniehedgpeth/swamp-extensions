...
/**
 * Swamp extension model for RefactorSpaces Environment (AWS::RefactorSpaces::Environment).
 *
...
/** Swamp extension model for RefactorSpaces Environment. Registered at `@swamp/aws/refactorspaces/environment`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List RefactorSpaces Environment resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.EnvironmentIdentifier?.toString() ??
