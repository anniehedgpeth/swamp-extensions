...
/**
 * Swamp extension model for MediaConnect FlowSource (AWS::MediaConnect::FlowSource).
 *
...
/** Swamp extension model for MediaConnect FlowSource. Registered at `@swamp/aws/mediaconnect/flow-source`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List MediaConnect FlowSource resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SourceArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
