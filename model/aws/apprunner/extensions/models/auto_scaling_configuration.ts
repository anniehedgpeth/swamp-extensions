...
/**
 * Swamp extension model for AppRunner AutoScalingConfiguration (AWS::AppRunner::AutoScalingConfiguration).
 *
...
/** Swamp extension model for AppRunner AutoScalingConfiguration. Registered at `@swamp/aws/apprunner/auto-scaling-configuration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppRunner AutoScalingConfiguration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AutoScalingConfigurationArn?.toString() ??
