...
/**
 * Swamp extension model for AppRunner ObservabilityConfiguration (AWS::AppRunner::ObservabilityConfiguration).
 *
...
/** Swamp extension model for AppRunner ObservabilityConfiguration. Registered at `@swamp/aws/apprunner/observability-configuration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppRunner ObservabilityConfiguration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ObservabilityConfigurationArn?.toString() ??
