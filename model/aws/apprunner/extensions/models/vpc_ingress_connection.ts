...
/**
 * Swamp extension model for AppRunner VpcIngressConnection (AWS::AppRunner::VpcIngressConnection).
 *
...
/** Swamp extension model for AppRunner VpcIngressConnection. Registered at `@swamp/aws/apprunner/vpc-ingress-connection`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppRunner VpcIngressConnection resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.VpcIngressConnectionArn?.toString() ??
