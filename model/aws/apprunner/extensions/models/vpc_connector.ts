...
/**
 * Swamp extension model for AppRunner VpcConnector (AWS::AppRunner::VpcConnector).
 *
...
/** Swamp extension model for AppRunner VpcConnector. Registered at `@swamp/aws/apprunner/vpc-connector`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppRunner VpcConnector resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.VpcConnectorArn?.toString() ?? item.identifier)
