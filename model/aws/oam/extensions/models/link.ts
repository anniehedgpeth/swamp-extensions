...
/**
 * Swamp extension model for Oam Link (AWS::Oam::Link).
 *
...
/** Swamp extension model for Oam Link. Registered at `@swamp/aws/oam/link`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Oam Link resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::Oam::Link", {
