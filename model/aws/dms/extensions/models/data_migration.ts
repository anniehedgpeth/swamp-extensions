...
/**
 * Swamp extension model for DMS DataMigration (AWS::DMS::DataMigration).
 *
...
const SourceDataSettingsSchema = z.object({
...
/** Swamp extension model for DMS DataMigration. Registered at `@swamp/aws/dms/data-migration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DMS DataMigration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DataMigrationArn?.toString() ?? item.identifier)
