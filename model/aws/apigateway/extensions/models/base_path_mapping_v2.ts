...
/**
 * Swamp extension model for ApiGateway BasePathMappingV2 (AWS::ApiGateway::BasePathMappingV2).
 *
...
/** Swamp extension model for ApiGateway BasePathMappingV2. Registered at `@swamp/aws/apigateway/base-path-mapping-v2`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ApiGateway BasePathMappingV2 resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.BasePathMappingArn?.toString() ?? item.identifier)
