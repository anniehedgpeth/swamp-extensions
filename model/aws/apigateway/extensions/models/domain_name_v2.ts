...
/**
 * Swamp extension model for ApiGateway DomainNameV2 (AWS::ApiGateway::DomainNameV2).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ApiGateway DomainNameV2 resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DomainNameArn?.toString() ?? item.identifier)
