...
/**
 * Swamp extension model for ApiGateway DomainNameAccessAssociation (AWS::ApiGateway::DomainNameAccessAssociation).
 *
...
/** Swamp extension model for ApiGateway DomainNameAccessAssociation. Registered at `@swamp/aws/apigateway/domain-name-access-association`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ApiGateway DomainNameAccessAssociation resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DomainNameAccessAssociationArn?.toString() ??
