...
/**
 * Swamp extension model for IoTWireless PartnerAccount (AWS::IoTWireless::PartnerAccount).
 *
...
/** Swamp extension model for IoTWireless PartnerAccount. Registered at `@swamp/aws/iotwireless/partner-account`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoTWireless PartnerAccount resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PartnerAccountId?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
