...
/**
 * Swamp extension model for AppStream DirectoryConfig (AWS::AppStream::DirectoryConfig).
 *
...
/** Swamp extension model for AppStream DirectoryConfig. Registered at `@swamp/aws/appstream/directory-config`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppStream DirectoryConfig resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DirectoryName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
