...
/**
 * Swamp extension model for Route53 DNSSEC (AWS::Route53::DNSSEC).
 *
...
/** Swamp extension model for Route53 DNSSEC. Registered at `@swamp/aws/route53/dnssec`. */
export const model = {
  ...
  methods: {
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.HostedZoneId?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    sync: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.HostedZoneId?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
    ...
    list: {
      description: "List Route53 DNSSEC resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.HostedZoneId?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
