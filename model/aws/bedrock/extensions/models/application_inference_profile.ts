...
/**
 * Swamp extension model for Bedrock ApplicationInferenceProfile (AWS::Bedrock::ApplicationInferenceProfile).
 *
...
/** Swamp extension model for Bedrock ApplicationInferenceProfile. Registered at `@swamp/aws/bedrock/application-inference-profile`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Bedrock ApplicationInferenceProfile resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.InferenceProfileIdentifier?.toString() ??
