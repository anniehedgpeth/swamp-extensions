...
/**
 * Swamp extension model for Backup BackupVault (AWS::Backup::BackupVault).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Backup BackupVault resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.BackupVaultName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
