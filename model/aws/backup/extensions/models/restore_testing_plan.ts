...
/**
 * Swamp extension model for Backup RestoreTestingPlan (AWS::Backup::RestoreTestingPlan).
 *
...
/** Swamp extension model for Backup RestoreTestingPlan. Registered at `@swamp/aws/backup/restore-testing-plan`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Backup RestoreTestingPlan resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RestoreTestingPlanName?.toString() ??
