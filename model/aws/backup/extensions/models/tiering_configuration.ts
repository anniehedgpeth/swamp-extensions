...
/**
 * Swamp extension model for Backup TieringConfiguration (AWS::Backup::TieringConfiguration).
 *
...
/** Swamp extension model for Backup TieringConfiguration. Registered at `@swamp/aws/backup/tiering-configuration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Backup TieringConfiguration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TieringConfigurationName?.toString() ??
