...
/**
 * Swamp extension model for Forecast DatasetGroup (AWS::Forecast::DatasetGroup).
 *
...
/** Swamp extension model for Forecast DatasetGroup. Registered at `@swamp/aws/forecast/dataset-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Forecast DatasetGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DatasetGroupArn?.toString() ?? item.identifier)
