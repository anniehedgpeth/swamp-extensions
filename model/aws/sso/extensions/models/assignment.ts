...
// Auto-generated extension model for @swamp/aws/sso/assignment
// Do not edit manually. Re-generate with: deno task generate:aws

// deno-lint-ignore-file no-explicit-any

/**
 * Swamp extension model for SSO Assignment (AWS::SSO::Assignment).
 *
...
const GlobalArgsSchema = z.object({
  ...
  TargetType: z.enum(["AWS_ACCOUNT"]).describe(
  ...
  PrincipalId: z.string().min(1).max(47).regex(
    new RegExp(
      "^([0-9a-f]{10}-|)[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}$",
    ),
  ).describe("The assignee's identifier, user id/group id"),
});
...
const InputsSchema = z.object({
  ...
  ).describe("The assignee's identifier, user id/group id").optional(),
});
...
/** Swamp extension model for SSO Assignment. Registered at `@swamp/aws/sso/assignment`. */
export const model = {
  ...
  resources: {
    state: {
      description: "SSO Assignment resource state",
      schema: StateSchema,
      lifetime: "infinite",
  ...
  methods: {
    create: {
      description: "Create a SSO Assignment",
      arguments: z.object({}),
    ...
    get: {
      description: "Get a SSO Assignment",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the SSO Assignment",
        ),
      }),
    ...
    delete: {
      description: "Delete a SSO Assignment",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the SSO Assignment",
        ),
    ...
    sync: {
      description: "Sync SSO Assignment state from AWS",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        ...
        const idParts = [
          ...
          existing.TargetType?.toString(),
          existing.PermissionSetArn?.toString(),
          existing.PrincipalType?.toString(),
          existing.PrincipalId?.toString(),
        ];
        if (idParts.some((p) => !p)) {
          throw new Error(
    ...
    list: {
      description: "List SSO Assignment resources",
      arguments: z.object({
