...
/**
 * Swamp extension model for MWAAServerless Workflow (AWS::MWAAServerless::Workflow).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List MWAAServerless Workflow resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.WorkflowArn?.toString() ?? item.identifier)
