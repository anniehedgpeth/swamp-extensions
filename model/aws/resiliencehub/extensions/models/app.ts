...
/**
 * Swamp extension model for ResilienceHub App (AWS::ResilienceHub::App).
 *
...
const PhysicalResourceIdSchema = z.object({
...
/** Swamp extension model for ResilienceHub App. Registered at `@swamp/aws/resiliencehub/app`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ResilienceHub App resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AppArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
