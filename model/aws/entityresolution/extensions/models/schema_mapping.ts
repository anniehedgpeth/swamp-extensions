...
/**
 * Swamp extension model for EntityResolution SchemaMapping (AWS::EntityResolution::SchemaMapping).
 *
...
const SchemaInputAttributeSchema = z.object({
...
/** Swamp extension model for EntityResolution SchemaMapping. Registered at `@swamp/aws/entityresolution/schema-mapping`. */
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const instanceName =
          ((result.SchemaName ?? g.SchemaName)?.toString() ?? "current")
    ...
    update: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.SchemaName?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.SchemaName?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List EntityResolution SchemaMapping resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SchemaName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
