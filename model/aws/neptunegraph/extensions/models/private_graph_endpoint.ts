...
/**
 * Swamp extension model for NeptuneGraph PrivateGraphEndpoint (AWS::NeptuneGraph::PrivateGraphEndpoint).
 *
...
/** Swamp extension model for NeptuneGraph PrivateGraphEndpoint. Registered at `@swamp/aws/neptunegraph/private-graph-endpoint`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List NeptuneGraph PrivateGraphEndpoint resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PrivateGraphEndpointIdentifier?.toString() ??
