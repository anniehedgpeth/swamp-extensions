...
/**
 * Swamp extension model for APS ResourcePolicy (AWS::APS::ResourcePolicy).
 *
...
/** Swamp extension model for APS ResourcePolicy. Registered at `@swamp/aws/aps/resource-policy`. */
export const model = {
  ...
  methods: {
    ...
    update: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.WorkspaceArn?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.WorkspaceArn?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List APS ResourcePolicy resources",
      arguments: z.object({
