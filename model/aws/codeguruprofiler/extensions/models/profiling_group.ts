...
/**
 * Swamp extension model for CodeGuruProfiler ProfilingGroup (AWS::CodeGuruProfiler::ProfilingGroup).
 *
...
/** Swamp extension model for CodeGuruProfiler ProfilingGroup. Registered at `@swamp/aws/codeguruprofiler/profiling-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CodeGuruProfiler ProfilingGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ProfilingGroupName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
