...
/**
 * Swamp extension model for WorkSpaces WorkspacesPool (AWS::WorkSpaces::WorkspacesPool).
 *
...
/** Swamp extension model for WorkSpaces WorkspacesPool. Registered at `@swamp/aws/workspaces/workspaces-pool`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpaces WorkspacesPool resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PoolId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
