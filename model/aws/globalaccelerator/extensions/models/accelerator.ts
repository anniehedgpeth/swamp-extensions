...
/**
 * Swamp extension model for GlobalAccelerator Accelerator (AWS::GlobalAccelerator::Accelerator).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List GlobalAccelerator Accelerator resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AcceleratorArn?.toString() ?? item.identifier)
