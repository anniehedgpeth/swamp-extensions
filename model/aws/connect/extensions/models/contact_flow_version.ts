...
/**
 * Swamp extension model for Connect ContactFlowVersion (AWS::Connect::ContactFlowVersion).
 *
...
/** Swamp extension model for Connect ContactFlowVersion. Registered at `@swamp/aws/connect/contact-flow-version`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect ContactFlowVersion resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ContactFlowVersionARN?.toString() ??
