...
/**
 * Swamp extension model for Connect SecurityProfile (AWS::Connect::SecurityProfile).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect SecurityProfile resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SecurityProfileArn?.toString() ?? item.identifier)
