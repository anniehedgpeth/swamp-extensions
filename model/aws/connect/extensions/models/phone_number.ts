...
/**
 * Swamp extension model for Connect PhoneNumber (AWS::Connect::PhoneNumber).
 *
...
/** Swamp extension model for Connect PhoneNumber. Registered at `@swamp/aws/connect/phone-number`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect PhoneNumber resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PhoneNumberArn?.toString() ?? item.identifier)
