...
/**
 * Swamp extension model for Connect QuickConnect (AWS::Connect::QuickConnect).
 *
...
const PhoneNumberQuickConnectConfigSchema = z.object({
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect QuickConnect resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.QuickConnectArn?.toString() ?? item.identifier)
