...
/**
 * Swamp extension model for Connect AgentStatus (AWS::Connect::AgentStatus).
 *
...
/** Swamp extension model for Connect AgentStatus. Registered at `@swamp/aws/connect/agent-status`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect AgentStatus resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AgentStatusArn?.toString() ?? item.identifier)
