...
/**
 * Swamp extension model for Connect TrafficDistributionGroup (AWS::Connect::TrafficDistributionGroup).
 *
...
/** Swamp extension model for Connect TrafficDistributionGroup. Registered at `@swamp/aws/connect/traffic-distribution-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect TrafficDistributionGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TrafficDistributionGroupArn?.toString() ??
