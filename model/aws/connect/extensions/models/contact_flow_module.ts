...
/**
 * Swamp extension model for Connect ContactFlowModule (AWS::Connect::ContactFlowModule).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect ContactFlowModule resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ContactFlowModuleArn?.toString() ??
