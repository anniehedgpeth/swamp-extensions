...
/**
 * Swamp extension model for Connect ContactFlowModuleAlias (AWS::Connect::ContactFlowModuleAlias).
 *
...
/** Swamp extension model for Connect ContactFlowModuleAlias. Registered at `@swamp/aws/connect/contact-flow-module-alias`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect ContactFlowModuleAlias resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ContactFlowModuleAliasARN?.toString() ??
