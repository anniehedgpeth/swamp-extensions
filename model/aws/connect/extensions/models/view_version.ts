...
/**
 * Swamp extension model for Connect ViewVersion (AWS::Connect::ViewVersion).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Connect ViewVersion resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ViewVersionArn?.toString() ?? item.identifier)
