...
/**
 * Swamp extension model for S3Vectors VectorBucketPolicy (AWS::S3Vectors::VectorBucketPolicy).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List S3Vectors VectorBucketPolicy resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.VectorBucketArn?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
