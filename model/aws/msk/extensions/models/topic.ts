...
/**
 * Swamp extension model for MSK Topic (AWS::MSK::Topic).
 *
...
/** Swamp extension model for MSK Topic. Registered at `@swamp/aws/msk/topic`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List MSK Topic resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::MSK::Topic", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TopicArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
