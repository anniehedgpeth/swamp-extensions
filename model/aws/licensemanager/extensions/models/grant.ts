...
/**
 * Swamp extension model for LicenseManager Grant (AWS::LicenseManager::Grant).
 *
...
/** Swamp extension model for LicenseManager Grant. Registered at `@swamp/aws/licensemanager/grant`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List LicenseManager Grant resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.GrantArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
