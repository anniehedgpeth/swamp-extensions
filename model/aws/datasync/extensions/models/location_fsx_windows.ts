...
/**
 * Swamp extension model for DataSync LocationFSxWindows (AWS::DataSync::LocationFSxWindows).
 *
...
/** Swamp extension model for DataSync LocationFSxWindows. Registered at `@swamp/aws/datasync/location-fsx-windows`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DataSync LocationFSxWindows resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.LocationArn?.toString() ?? item.identifier)
