...
/**
 * Swamp extension model for Logs QueryDefinition (AWS::Logs::QueryDefinition).
 *
...
/** Swamp extension model for Logs QueryDefinition. Registered at `@swamp/aws/logs/query-definition`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Logs QueryDefinition resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.QueryDefinitionId?.toString() ?? item.identifier)
