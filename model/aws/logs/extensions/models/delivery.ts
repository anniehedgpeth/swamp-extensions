...
/**
 * Swamp extension model for Logs Delivery (AWS::Logs::Delivery).
 *
...
/** Swamp extension model for Logs Delivery. Registered at `@swamp/aws/logs/delivery`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Logs Delivery resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DeliveryId?.toString() ?? item.identifier)
