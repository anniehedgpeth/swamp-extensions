...
/**
 * Swamp extension model for AppFlow Connector (AWS::AppFlow::Connector).
 *
...
const LambdaConnectorProvisioningConfigSchema = z.object({
...
/** Swamp extension model for AppFlow Connector. Registered at `@swamp/aws/appflow/connector`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List AppFlow Connector resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ConnectorLabel?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
