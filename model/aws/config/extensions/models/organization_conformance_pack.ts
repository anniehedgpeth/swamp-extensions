...
/**
 * Swamp extension model for Config OrganizationConformancePack (AWS::Config::OrganizationConformancePack).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Config OrganizationConformancePack resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.OrganizationConformancePackName?.toString() ??
