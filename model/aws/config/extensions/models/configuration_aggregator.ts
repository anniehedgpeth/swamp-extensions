...
/**
 * Swamp extension model for Config ConfigurationAggregator (AWS::Config::ConfigurationAggregator).
 *
...
const AccountAggregationSourceSchema = z.object({
...
/** Swamp extension model for Config ConfigurationAggregator. Registered at `@swamp/aws/config/configuration-aggregator`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Config ConfigurationAggregator resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ConfigurationAggregatorName?.toString() ??
