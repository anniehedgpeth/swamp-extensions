...
/**
 * Swamp extension model for SageMaker DeviceFleet (AWS::SageMaker::DeviceFleet).
 *
...
/** Swamp extension model for SageMaker DeviceFleet. Registered at `@swamp/aws/sagemaker/device-fleet`. */
export const model = {
  ...
  methods: {
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.DeviceFleetName?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
