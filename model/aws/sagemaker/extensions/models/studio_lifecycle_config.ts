...
/**
 * Swamp extension model for SageMaker StudioLifecycleConfig (AWS::SageMaker::StudioLifecycleConfig).
 *
...
/** Swamp extension model for SageMaker StudioLifecycleConfig. Registered at `@swamp/aws/sagemaker/studio-lifecycle-config`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SageMaker StudioLifecycleConfig resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.StudioLifecycleConfigName?.toString() ??
