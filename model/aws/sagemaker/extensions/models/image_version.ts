...
/**
 * Swamp extension model for SageMaker ImageVersion (AWS::SageMaker::ImageVersion).
 *
...
/** Swamp extension model for SageMaker ImageVersion. Registered at `@swamp/aws/sagemaker/image-version`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SageMaker ImageVersion resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ImageVersionArn?.toString() ?? item.identifier)
