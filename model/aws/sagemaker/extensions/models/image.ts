...
/**
 * Swamp extension model for SageMaker Image (AWS::SageMaker::Image).
 *
...
/** Swamp extension model for SageMaker Image. Registered at `@swamp/aws/sagemaker/image`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SageMaker Image resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ImageArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
