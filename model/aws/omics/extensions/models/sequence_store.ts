...
/**
 * Swamp extension model for Omics SequenceStore (AWS::Omics::SequenceStore).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Omics SequenceStore resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SequenceStoreId?.toString() ?? item.identifier)
