...
/**
 * Swamp extension model for Route53RecoveryReadiness Cell (AWS::Route53RecoveryReadiness::Cell).
 *
...
/** Swamp extension model for Route53RecoveryReadiness Cell. Registered at `@swamp/aws/route53recoveryreadiness/cell`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53RecoveryReadiness Cell resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.CellName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
