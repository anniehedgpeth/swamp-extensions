...
/**
 * Swamp extension model for Route53RecoveryReadiness ReadinessCheck (AWS::Route53RecoveryReadiness::ReadinessCheck).
 *
...
/** Swamp extension model for Route53RecoveryReadiness ReadinessCheck. Registered at `@swamp/aws/route53recoveryreadiness/readiness-check`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53RecoveryReadiness ReadinessCheck resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ReadinessCheckName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
