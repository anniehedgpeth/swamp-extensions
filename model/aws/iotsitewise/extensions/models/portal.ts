...
/**
 * Swamp extension model for IoTSiteWise Portal (AWS::IoTSiteWise::Portal).
 *
...
const PortalTypeEntrySchema = z.object({
...
/** Swamp extension model for IoTSiteWise Portal. Registered at `@swamp/aws/iotsitewise/portal`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoTSiteWise Portal resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PortalId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
