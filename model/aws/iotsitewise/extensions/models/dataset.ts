...
/**
 * Swamp extension model for IoTSiteWise Dataset (AWS::IoTSiteWise::Dataset).
 *
...
const KendraSourceDetailSchema = z.object({
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoTSiteWise Dataset resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DatasetId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
