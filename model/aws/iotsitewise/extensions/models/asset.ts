...
/**
 * Swamp extension model for IoTSiteWise Asset (AWS::IoTSiteWise::Asset).
 *
...
const AssetHierarchySchema = z.object({
  Id: z.string().min(36).max(36).regex(
    new RegExp(
      "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
    ),
...
/** Swamp extension model for IoTSiteWise Asset. Registered at `@swamp/aws/iotsitewise/asset`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoTSiteWise Asset resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AssetId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
