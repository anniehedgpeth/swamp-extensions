...
/**
 * Swamp extension model for IoTTwinMaker Workspace (AWS::IoTTwinMaker::Workspace).
 *
...
/** Swamp extension model for IoTTwinMaker Workspace. Registered at `@swamp/aws/iottwinmaker/workspace`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoTTwinMaker Workspace resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.WorkspaceId?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
