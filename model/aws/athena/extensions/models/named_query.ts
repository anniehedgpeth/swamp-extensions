...
/**
 * Swamp extension model for Athena NamedQuery (AWS::Athena::NamedQuery).
 *
...
/** Swamp extension model for Athena NamedQuery. Registered at `@swamp/aws/athena/named-query`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Athena NamedQuery resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.NamedQueryId?.toString() ?? item.identifier)
