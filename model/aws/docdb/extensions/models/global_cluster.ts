...
/**
 * Swamp extension model for DocDB GlobalCluster (AWS::DocDB::GlobalCluster).
 *
...
/** Swamp extension model for DocDB GlobalCluster. Registered at `@swamp/aws/docdb/global-cluster`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DocDB GlobalCluster resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.GlobalClusterIdentifier?.toString() ??
