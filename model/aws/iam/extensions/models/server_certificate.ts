...
/**
 * Swamp extension model for IAM ServerCertificate (AWS::IAM::ServerCertificate).
 *
...
/** Swamp extension model for IAM ServerCertificate. Registered at `@swamp/aws/iam/server-certificate`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IAM ServerCertificate resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ServerCertificateName?.toString() ??
