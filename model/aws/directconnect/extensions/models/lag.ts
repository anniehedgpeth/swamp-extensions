...
/**
 * Swamp extension model for DirectConnect Lag (AWS::DirectConnect::Lag).
 *
...
/** Swamp extension model for DirectConnect Lag. Registered at `@swamp/aws/directconnect/lag`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DirectConnect Lag resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.LagArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
