...
/**
 * Swamp extension model for DirectConnect DirectConnectGateway (AWS::DirectConnect::DirectConnectGateway).
 *
...
/** Swamp extension model for DirectConnect DirectConnectGateway. Registered at `@swamp/aws/directconnect/direct-connect-gateway`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DirectConnect DirectConnectGateway resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DirectConnectGatewayArn?.toString() ??
