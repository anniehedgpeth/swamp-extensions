...
/**
 * Swamp extension model for Wisdom QuickResponse (AWS::Wisdom::QuickResponse).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Wisdom QuickResponse resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.QuickResponseArn?.toString() ?? item.identifier)
