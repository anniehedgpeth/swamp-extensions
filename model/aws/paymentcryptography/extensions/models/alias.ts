...
/**
 * Swamp extension model for PaymentCryptography Alias (AWS::PaymentCryptography::Alias).
 *
...
/** Swamp extension model for PaymentCryptography Alias. Registered at `@swamp/aws/paymentcryptography/alias`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List PaymentCryptography Alias resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AliasName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
