...
/**
 * Swamp extension model for SecurityHub PolicyAssociation (AWS::SecurityHub::PolicyAssociation).
 *
...
/** Swamp extension model for SecurityHub PolicyAssociation. Registered at `@swamp/aws/securityhub/policy-association`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SecurityHub PolicyAssociation resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AssociationIdentifier?.toString() ??
