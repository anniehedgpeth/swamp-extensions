...
/**
 * Swamp extension model for FMS NotificationChannel (AWS::FMS::NotificationChannel).
 *
...
/** Swamp extension model for FMS NotificationChannel. Registered at `@swamp/aws/fms/notification-channel`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List FMS NotificationChannel resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SnsTopicArn?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
