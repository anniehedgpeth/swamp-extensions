...
/**
 * Swamp extension model for ControlTower EnabledBaseline (AWS::ControlTower::EnabledBaseline).
 *
...
/** Swamp extension model for ControlTower EnabledBaseline. Registered at `@swamp/aws/controltower/enabled-baseline`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ControlTower EnabledBaseline resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.EnabledBaselineIdentifier?.toString() ??
