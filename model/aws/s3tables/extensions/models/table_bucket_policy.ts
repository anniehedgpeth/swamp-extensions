...
/**
 * Swamp extension model for S3Tables TableBucketPolicy (AWS::S3Tables::TableBucketPolicy).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List S3Tables TableBucketPolicy resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TableBucketARN?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
