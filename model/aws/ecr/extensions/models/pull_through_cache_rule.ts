...
/**
 * Swamp extension model for ECR PullThroughCacheRule (AWS::ECR::PullThroughCacheRule).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ECR PullThroughCacheRule resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.EcrRepositoryPrefix?.toString() ??
