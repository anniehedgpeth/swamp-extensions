...
/**
 * Swamp extension model for Invoicing InvoiceUnit (AWS::Invoicing::InvoiceUnit).
 *
...
/** Swamp extension model for Invoicing InvoiceUnit. Registered at `@swamp/aws/invoicing/invoice-unit`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Invoicing InvoiceUnit resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.InvoiceUnitArn?.toString() ?? item.identifier)
