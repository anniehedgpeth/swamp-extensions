...
/**
 * Swamp extension model for S3 MultiRegionAccessPointPolicy (AWS::S3::MultiRegionAccessPointPolicy).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List S3 MultiRegionAccessPointPolicy resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.MrapName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
