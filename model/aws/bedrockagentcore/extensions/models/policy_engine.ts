...
/**
 * Swamp extension model for BedrockAgentCore PolicyEngine (AWS::BedrockAgentCore::PolicyEngine).
 *
...
/** Swamp extension model for BedrockAgentCore PolicyEngine. Registered at `@swamp/aws/bedrockagentcore/policy-engine`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List BedrockAgentCore PolicyEngine resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PolicyEngineArn?.toString() ?? item.identifier)
