...
/**
 * Swamp extension model for SES CustomVerificationEmailTemplate (AWS::SES::CustomVerificationEmailTemplate).
 *
...
/** Swamp extension model for SES CustomVerificationEmailTemplate. Registered at `@swamp/aws/ses/custom-verification-email-template`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SES CustomVerificationEmailTemplate resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TemplateName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
