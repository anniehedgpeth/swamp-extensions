...
/**
 * Swamp extension model for SES DedicatedIpPool (AWS::SES::DedicatedIpPool).
 *
...
/** Swamp extension model for SES DedicatedIpPool. Registered at `@swamp/aws/ses/dedicated-ip-pool`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SES DedicatedIpPool resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PoolName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
