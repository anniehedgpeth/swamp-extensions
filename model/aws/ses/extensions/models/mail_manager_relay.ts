...
/**
 * Swamp extension model for SES MailManagerRelay (AWS::SES::MailManagerRelay).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SES MailManagerRelay resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RelayId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
