...
/**
 * Swamp extension model for SES MailManagerArchive (AWS::SES::MailManagerArchive).
 *
...
/** Swamp extension model for SES MailManagerArchive. Registered at `@swamp/aws/ses/mail-manager-archive`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SES MailManagerArchive resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ArchiveId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
