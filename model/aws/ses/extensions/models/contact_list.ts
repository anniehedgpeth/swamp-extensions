...
/**
 * Swamp extension model for SES ContactList (AWS::SES::ContactList).
 *
...
/** Swamp extension model for SES ContactList. Registered at `@swamp/aws/ses/contact-list`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SES ContactList resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ContactListName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
