...
/**
 * Swamp extension model for PCAConnectorAD DirectoryRegistration (AWS::PCAConnectorAD::DirectoryRegistration).
 *
...
/** Swamp extension model for PCAConnectorAD DirectoryRegistration. Registered at `@swamp/aws/pcaconnectorad/directory-registration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List PCAConnectorAD DirectoryRegistration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DirectoryRegistrationArn?.toString() ??
