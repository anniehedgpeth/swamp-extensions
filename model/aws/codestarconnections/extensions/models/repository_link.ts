...
/**
 * Swamp extension model for CodeStarConnections RepositoryLink (AWS::CodeStarConnections::RepositoryLink).
 *
...
/** Swamp extension model for CodeStarConnections RepositoryLink. Registered at `@swamp/aws/codestarconnections/repository-link`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CodeStarConnections RepositoryLink resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RepositoryLinkArn?.toString() ?? item.identifier)
