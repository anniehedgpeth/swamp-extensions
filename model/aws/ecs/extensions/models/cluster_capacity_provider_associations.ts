...
/**
 * Swamp extension model for ECS ClusterCapacityProviderAssociations (AWS::ECS::ClusterCapacityProviderAssociations).
 *
...
const CapacityProviderStrategySchema = z.object({
...
/** Swamp extension model for ECS ClusterCapacityProviderAssociations. Registered at `@swamp/aws/ecs/cluster-capacity-provider-associations`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ECS ClusterCapacityProviderAssociations resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.Cluster?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
