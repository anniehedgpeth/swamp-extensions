...
// Auto-generated extension model for @swamp/aws/ecs/service
...
const PlacementStrategySchema = z.object({
  ...
  Type: z.enum(["binpack", "random", "spread"]).describe(
...
const EBSTagSpecificationSchema = z.object({
  ...
  ResourceType: z.string().describe("The type of volume resource."),
...
const ServiceManagedEBSVolumeConfigurationSchema = z.object({
  ...
  TagSpecifications: z.array(EBSTagSpecificationSchema).describe(
  ...
  FilesystemType: z.string().describe(
...
const LoadBalancerSchema = z.object({
  ...
  AdvancedConfiguration: AdvancedConfigurationSchema.describe(
...
const TimeoutConfigurationSchema = z.object({
  ...
  IdleTimeoutSeconds: z.number().int().describe(
...
const ServiceConnectClientAliasSchema = z.object({
  ...
  TestTrafficRules: ServiceConnectTestTrafficRulesSchema.describe(
...
const ServiceConnectServiceSchema = z.object({
  ...
  IngressPortOverride: z.number().int().describe(
  ...
  ClientAliases: z.array(ServiceConnectClientAliasSchema).describe(
  ...
  Tls: ServiceConnectTlsConfigurationSchema.describe(
    "A reference to an object that represents a Transport Layer Security (TLS) configuration.",
  ).optional(),
  DiscoveryName: z.string().describe(
...
const ServiceConnectAccessLogConfigurationSchema = z.object({
  ...
  IncludeQueryParameters: z.enum(["DISABLED", "ENABLED"]).describe(
...
const CanaryConfigurationSchema = z.object({
  ...
  CanaryBakeTimeInMinutes: z.number().int().min(0).max(1440).describe(
...
const LinearConfigurationSchema = z.object({
  ...
  StepPercent: z.number().min(3).max(100).describe(
...
const GlobalArgsSchema = z.object({
  ...
  PlacementStrategies: z.array(PlacementStrategySchema).describe(
  ...
  VolumeConfigurations: z.array(ServiceVolumeConfigurationSchema).describe(
  ...
  AvailabilityZoneRebalancing: z.enum(["ENABLED", "DISABLED"]).describe(
  ...
  SchedulingStrategy: z.enum(["DAEMON", "REPLICA"]).describe(
  ...
  ForceNewDeployment: z.object({
    ...
    ForceNewDeploymentNonce: z.string().min(1).max(255).describe(
  ...
  HealthCheckGracePeriodSeconds: z.number().int().describe(
  ...
  LoadBalancers: z.array(LoadBalancerSchema).describe(
  ...
  ServiceConnectConfiguration: z.object({
    Services: z.array(ServiceConnectServiceSchema).describe(
    ...
    AccessLogConfiguration: ServiceConnectAccessLogConfigurationSchema.describe(
  ...
  DesiredCount: z.number().int().describe(
  ...
  VpcLatticeConfigurations: z.array(VpcLatticeConfigurationSchema).describe(
    "The VPC Lattice configuration for the service being created.",
  ).optional(),
  DeploymentController: z.object({
    Type: z.enum(["CODE_DEPLOY", "ECS", "EXTERNAL"]).describe(
  ...
  }).describe("The deployment controller to use for the service.").optional(),
  ...
  DeploymentConfiguration: z.object({
    CanaryConfiguration: CanaryConfigurationSchema.describe(
    ...
    Strategy: z.enum(["ROLLING", "BLUE_GREEN", "LINEAR", "CANARY"]).describe(
    ...
    DeploymentCircuitBreaker: DeploymentCircuitBreakerSchema.describe(
    ...
    MaximumPercent: z.number().int().describe(
    ...
    MinimumHealthyPercent: z.number().int().describe(
    ...
    LinearConfiguration: LinearConfigurationSchema.describe(
...
const StateSchema = z.object({
  ...
  VolumeConfigurations: z.array(ServiceVolumeConfigurationSchema).optional(),
  CapacityProviderStrategy: z.array(CapacityProviderStrategyItemSchema)
    .optional(),
  LaunchType: z.string().optional(),
  Name: z.string().optional(),
  AvailabilityZoneRebalancing: z.string().optional(),
  ...
  HealthCheckGracePeriodSeconds: z.number().optional(),
  ...
  ServiceConnectConfiguration: z.object({
    Services: z.array(ServiceConnectServiceSchema),
    AccessLogConfiguration: ServiceConnectAccessLogConfigurationSchema,
    Enabled: z.boolean(),
    LogConfiguration: LogConfigurationSchema,
    Namespace: z.string(),
  }).optional(),
  DesiredCount: z.number().optional(),
  VpcLatticeConfigurations: z.array(VpcLatticeConfigurationSchema).optional(),
  DeploymentController: z.object({
    Type: z.string(),
  }).optional(),
  ...
  DeploymentConfiguration: z.object({
    ...
    MinimumHealthyPercent: z.number(),
    LinearConfiguration: LinearConfigurationSchema,
  }).optional(),
}).passthrough();
...
export const model = {
  ...
  resources: {
    state: {
      description: "ECS Service resource state",
      schema: StateSchema,
      lifetime: "infinite",
  ...
  methods: {
    create: {
      description: "Create a ECS Service",
      arguments: z.object({}),
    ...
    get: {
      description: "Get a ECS Service",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the ECS Service",
        ),
      }),
    ...
    update: {
      description: "Update a ECS Service",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        ...
        const result = await updateResource(
          ...
          currentState,
          desiredState,
          ["Cluster", "Role", "SchedulingStrategy", "ServiceName"],
          credentials,
        );
        const handle = await context.writeResource(
          "state",
    ...
    delete: {
      description: "Delete a ECS Service",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the ECS Service",
        ),
    ...
    sync: {
      description: "Sync ECS Service state from AWS",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
    ...
    list: {
      description: "List ECS Service resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::ECS::Service", {
