...
/**
 * Swamp extension model for Lightsail StaticIp (AWS::Lightsail::StaticIp).
 *
...
/** Swamp extension model for Lightsail StaticIp. Registered at `@swamp/aws/lightsail/static-ip`. */
export const model = {
  ...
  methods: {
    ...
    update: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.StaticIpName?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.StaticIpName?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List Lightsail StaticIp resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.StaticIpName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
