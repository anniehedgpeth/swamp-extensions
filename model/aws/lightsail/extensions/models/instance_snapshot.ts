...
/**
 * Swamp extension model for Lightsail InstanceSnapshot (AWS::Lightsail::InstanceSnapshot).
 *
...
/** Swamp extension model for Lightsail InstanceSnapshot. Registered at `@swamp/aws/lightsail/instance-snapshot`. */
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const instanceName =
          ((result.InstanceSnapshotName ?? g.InstanceSnapshotName)
    ...
    list: {
      description: "List Lightsail InstanceSnapshot resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.InstanceSnapshotName?.toString() ??
