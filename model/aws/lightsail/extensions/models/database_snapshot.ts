...
/**
 * Swamp extension model for Lightsail DatabaseSnapshot (AWS::Lightsail::DatabaseSnapshot).
 *
...
/** Swamp extension model for Lightsail DatabaseSnapshot. Registered at `@swamp/aws/lightsail/database-snapshot`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Lightsail DatabaseSnapshot resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RelationalDatabaseSnapshotName?.toString() ??
