...
/**
 * Swamp extension model for Lightsail Disk (AWS::Lightsail::Disk).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Lightsail Disk resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DiskName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
