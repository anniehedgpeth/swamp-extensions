...
/**
 * Swamp extension model for EC2 VPC (AWS::EC2::VPC).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 VPC resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::EC2::VPC", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.VpcId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
