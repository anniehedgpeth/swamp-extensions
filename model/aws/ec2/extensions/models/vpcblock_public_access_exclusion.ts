...
/**
 * Swamp extension model for EC2 VPCBlockPublicAccessExclusion (AWS::EC2::VPCBlockPublicAccessExclusion).
 *
...
/** Swamp extension model for EC2 VPCBlockPublicAccessExclusion. Registered at `@swamp/aws/ec2/vpcblock-public-access-exclusion`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 VPCBlockPublicAccessExclusion resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ExclusionId?.toString() ?? item.identifier)
