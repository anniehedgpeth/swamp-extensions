...
/**
 * Swamp extension model for EC2 IPAMResourceDiscoveryAssociation (AWS::EC2::IPAMResourceDiscoveryAssociation).
 *
...
/** Swamp extension model for EC2 IPAMResourceDiscoveryAssociation. Registered at `@swamp/aws/ec2/ipamresource-discovery-association`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 IPAMResourceDiscoveryAssociation resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.IpamResourceDiscoveryAssociationId?.toString() ??
