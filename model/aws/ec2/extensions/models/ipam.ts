...
/**
 * Swamp extension model for EC2 IPAM (AWS::EC2::IPAM).
 *
...
const IpamOrganizationalUnitExclusionSchema = z.object({
...
/** Swamp extension model for EC2 IPAM. Registered at `@swamp/aws/ec2/ipam`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 IPAM resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::EC2::IPAM", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.IpamId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
