...
/**
 * Swamp extension model for EC2 TrafficMirrorFilterRule (AWS::EC2::TrafficMirrorFilterRule).
 *
...
/** Swamp extension model for EC2 TrafficMirrorFilterRule. Registered at `@swamp/aws/ec2/traffic-mirror-filter-rule`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 TrafficMirrorFilterRule resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TrafficMirrorFilterRuleId?.toString() ??
