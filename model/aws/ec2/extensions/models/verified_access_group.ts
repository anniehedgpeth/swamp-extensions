...
/**
 * Swamp extension model for EC2 VerifiedAccessGroup (AWS::EC2::VerifiedAccessGroup).
 *
...
/** Swamp extension model for EC2 VerifiedAccessGroup. Registered at `@swamp/aws/ec2/verified-access-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 VerifiedAccessGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.VerifiedAccessGroupId?.toString() ??
