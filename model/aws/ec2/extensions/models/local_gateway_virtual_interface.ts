...
/**
 * Swamp extension model for EC2 LocalGatewayVirtualInterface (AWS::EC2::LocalGatewayVirtualInterface).
 *
...
/** Swamp extension model for EC2 LocalGatewayVirtualInterface. Registered at `@swamp/aws/ec2/local-gateway-virtual-interface`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 LocalGatewayVirtualInterface resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.LocalGatewayVirtualInterfaceId?.toString() ??
