...
/**
 * Swamp extension model for EC2 NetworkInsightsPath (AWS::EC2::NetworkInsightsPath).
 *
...
const FilterPortRangeSchema = z.object({
...
/** Swamp extension model for EC2 NetworkInsightsPath. Registered at `@swamp/aws/ec2/network-insights-path`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 NetworkInsightsPath resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.NetworkInsightsPathId?.toString() ??
