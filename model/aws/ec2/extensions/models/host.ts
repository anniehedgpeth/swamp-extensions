...
/**
 * Swamp extension model for EC2 Host (AWS::EC2::Host).
 *
...
/** Swamp extension model for EC2 Host. Registered at `@swamp/aws/ec2/host`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 Host resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::EC2::Host", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.HostId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
