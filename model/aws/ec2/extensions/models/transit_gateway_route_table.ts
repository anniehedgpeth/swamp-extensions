...
/**
 * Swamp extension model for EC2 TransitGatewayRouteTable (AWS::EC2::TransitGatewayRouteTable).
 *
...
/** Swamp extension model for EC2 TransitGatewayRouteTable. Registered at `@swamp/aws/ec2/transit-gateway-route-table`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 TransitGatewayRouteTable resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TransitGatewayRouteTableId?.toString() ??
