...
/**
 * Swamp extension model for EC2 TransitGatewayConnectPeer (AWS::EC2::TransitGatewayConnectPeer).
 *
...
/** Swamp extension model for EC2 TransitGatewayConnectPeer. Registered at `@swamp/aws/ec2/transit-gateway-connect-peer`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 TransitGatewayConnectPeer resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TransitGatewayConnectPeerId?.toString() ??
