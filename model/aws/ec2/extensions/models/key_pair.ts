...
// Auto-generated extension model for @swamp/aws/ec2/key-pair
// Do not edit manually. Re-generate with: deno task generate:aws

// deno-lint-ignore-file no-explicit-any

/**
 * Swamp extension model for EC2 KeyPair (AWS::EC2::KeyPair).
 *
...
const GlobalArgsSchema = z.object({
  ...
  KeyType: z.enum(["rsa", "ed25519"]).describe(
  ...
  KeyFormat: z.enum(["pem", "ppk"]).describe(
    "The format of the key pair. Default: pem",
  ).optional(),
  PublicKeyMaterial: z.string().describe(
  ...
  Tags: z.array(TagSchema).describe("The tags to apply to the key pair.")
    .optional(),
});
...
/** Swamp extension model for EC2 KeyPair. Registered at `@swamp/aws/ec2/key-pair`. */
export const model = {
  type: "@swamp/aws/ec2/key-pair",
  ...
  resources: {
    state: {
      description: "EC2 KeyPair resource state",
      schema: StateSchema,
      lifetime: "infinite",
  ...
  methods: {
    create: {
      description: "Create a EC2 KeyPair",
      arguments: z.object({}),
    ...
    get: {
      description: "Get a EC2 KeyPair",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the EC2 KeyPair",
        ),
      }),
    ...
    delete: {
      description: "Delete a EC2 KeyPair",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the EC2 KeyPair",
        ),
    ...
    sync: {
      description: "Sync EC2 KeyPair state from AWS",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
    ...
    list: {
      description: "List EC2 KeyPair resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::EC2::KeyPair", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.KeyName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
