...
/**
 * Swamp extension model for EC2 TransitGatewayMulticastDomain (AWS::EC2::TransitGatewayMulticastDomain).
 *
...
/** Swamp extension model for EC2 TransitGatewayMulticastDomain. Registered at `@swamp/aws/ec2/transit-gateway-multicast-domain`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List EC2 TransitGatewayMulticastDomain resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TransitGatewayMulticastDomainId?.toString() ??
