...
/**
 * Swamp extension model for LakeFormation Tag (AWS::LakeFormation::Tag).
 *
...
/** Swamp extension model for LakeFormation Tag. Registered at `@swamp/aws/lakeformation/tag`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List LakeFormation Tag resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TagKey?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
