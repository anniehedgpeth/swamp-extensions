...
/**
 * Swamp extension model for GameLift Build (AWS::GameLift::Build).
 *
...
/** Swamp extension model for GameLift Build. Registered at `@swamp/aws/gamelift/build`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List GameLift Build resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.BuildId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
