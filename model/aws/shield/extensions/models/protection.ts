...
/**
 * Swamp extension model for Shield Protection (AWS::Shield::Protection).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Shield Protection resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ProtectionArn?.toString() ?? item.identifier)
