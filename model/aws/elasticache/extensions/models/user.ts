...
/**
 * Swamp extension model for ElastiCache User (AWS::ElastiCache::User).
 *
...
/** Swamp extension model for ElastiCache User. Registered at `@swamp/aws/elasticache/user`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ElastiCache User resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.UserId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
