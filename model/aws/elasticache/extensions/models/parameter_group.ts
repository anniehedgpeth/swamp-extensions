...
/**
 * Swamp extension model for ElastiCache ParameterGroup (AWS::ElastiCache::ParameterGroup).
 *
...
/** Swamp extension model for ElastiCache ParameterGroup. Registered at `@swamp/aws/elasticache/parameter-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List ElastiCache ParameterGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.CacheParameterGroupName?.toString() ??
