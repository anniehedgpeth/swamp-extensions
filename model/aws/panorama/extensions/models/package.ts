...
/**
 * Swamp extension model for Panorama Package (AWS::Panorama::Package).
 *
...
/** Swamp extension model for Panorama Package. Registered at `@swamp/aws/panorama/package`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Panorama Package resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PackageId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
