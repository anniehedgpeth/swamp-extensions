...
/**
 * Swamp extension model for CE AnomalyMonitor (AWS::CE::AnomalyMonitor).
 *
...
/** Swamp extension model for CE AnomalyMonitor. Registered at `@swamp/aws/ce/anomaly-monitor`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CE AnomalyMonitor resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.MonitorArn?.toString() ?? item.identifier)
