...
/**
 * Swamp extension model for Route53GlobalResolver FirewallRule (AWS::Route53GlobalResolver::FirewallRule).
 *
...
/** Swamp extension model for Route53GlobalResolver FirewallRule. Registered at `@swamp/aws/route53globalresolver/firewall-rule`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53GlobalResolver FirewallRule resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.FirewallRuleId?.toString() ?? item.identifier)
