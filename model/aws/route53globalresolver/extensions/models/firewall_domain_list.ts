...
/**
 * Swamp extension model for Route53GlobalResolver FirewallDomainList (AWS::Route53GlobalResolver::FirewallDomainList).
 *
...
/** Swamp extension model for Route53GlobalResolver FirewallDomainList. Registered at `@swamp/aws/route53globalresolver/firewall-domain-list`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53GlobalResolver FirewallDomainList resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.FirewallDomainListId?.toString() ??
