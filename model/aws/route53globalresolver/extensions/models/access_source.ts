...
/**
 * Swamp extension model for Route53GlobalResolver AccessSource (AWS::Route53GlobalResolver::AccessSource).
 *
...
/** Swamp extension model for Route53GlobalResolver AccessSource. Registered at `@swamp/aws/route53globalresolver/access-source`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53GlobalResolver AccessSource resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AccessSourceId?.toString() ?? item.identifier)
