...
/**
 * Swamp extension model for Route53GlobalResolver GlobalResolver (AWS::Route53GlobalResolver::GlobalResolver).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Route53GlobalResolver GlobalResolver resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.GlobalResolverId?.toString() ?? item.identifier)
