...
/**
 * Swamp extension model for VerifiedPermissions PolicyStore (AWS::VerifiedPermissions::PolicyStore).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List VerifiedPermissions PolicyStore resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PolicyStoreId?.toString() ?? item.identifier)
