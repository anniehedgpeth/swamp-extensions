...
/**
 * Swamp extension model for RDS DBProxyTargetGroup (AWS::RDS::DBProxyTargetGroup).
 *
...
/** Swamp extension model for RDS DBProxyTargetGroup. Registered at `@swamp/aws/rds/dbproxy-target-group`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List RDS DBProxyTargetGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TargetGroupArn?.toString() ?? item.identifier)
