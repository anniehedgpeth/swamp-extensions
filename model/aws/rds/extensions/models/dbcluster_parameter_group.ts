...
/**
 * Swamp extension model for RDS DBClusterParameterGroup (AWS::RDS::DBClusterParameterGroup).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List RDS DBClusterParameterGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DBClusterParameterGroupName?.toString() ??
