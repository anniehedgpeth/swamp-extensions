...
/**
 * Swamp extension model for RDS DBParameterGroup (AWS::RDS::DBParameterGroup).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List RDS DBParameterGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DBParameterGroupName?.toString() ??
