...
// Auto-generated extension model for @swamp/aws/observabilityadmin/telemetry-rule
...
const GlobalArgsSchema = z.object({
  ...
  RuleName: z.string().min(1).max(100).regex(new RegExp("^[0-9A-Za-z-]+$"))
    .describe("The name of the telemetry rule"),
  Rule: z.object({
    ResourceType: z.enum([
      "AWS::EC2::VPC",
      "AWS::WAFv2::WebACL",
...
const InputsSchema = z.object({
  ...
  RuleName: z.string().min(1).max(100).regex(new RegExp("^[0-9A-Za-z-]+$"))
    .describe("The name of the telemetry rule").optional(),
  Rule: z.object({
    ResourceType: z.enum([
      "AWS::EC2::VPC",
      "AWS::WAFv2::WebACL",
...
export const model = {
  ...
  resources: {
    state: {
      description: "ObservabilityAdmin TelemetryRule resource state",
      schema: StateSchema,
      lifetime: "infinite",
  ...
  methods: {
    create: {
      description: "Create a ObservabilityAdmin TelemetryRule",
      arguments: z.object({}),
    ...
    get: {
      description: "Get a ObservabilityAdmin TelemetryRule",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the ObservabilityAdmin TelemetryRule",
        ),
      }),
    ...
    update: {
      description: "Update a ObservabilityAdmin TelemetryRule",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
    ...
    delete: {
      description: "Delete a ObservabilityAdmin TelemetryRule",
      arguments: z.object({
        identifier: z.string().describe(
          "The primary identifier of the ObservabilityAdmin TelemetryRule",
        ),
    ...
    sync: {
      description: "Sync ObservabilityAdmin TelemetryRule state from AWS",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
    ...
    list: {
      description: "List ObservabilityAdmin TelemetryRule resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RuleArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
