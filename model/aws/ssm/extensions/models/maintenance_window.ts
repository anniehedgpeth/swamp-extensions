...
/**
 * Swamp extension model for SSM MaintenanceWindow (AWS::SSM::MaintenanceWindow).
 *
...
/** Swamp extension model for SSM MaintenanceWindow. Registered at `@swamp/aws/ssm/maintenance-window`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SSM MaintenanceWindow resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.WindowId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
