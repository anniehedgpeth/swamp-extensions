...
/**
 * Swamp extension model for SSM ResourceDataSync (AWS::SSM::ResourceDataSync).
 *
...
const AwsOrganizationsSourceSchema = z.object({
...
/** Swamp extension model for SSM ResourceDataSync. Registered at `@swamp/aws/ssm/resource-data-sync`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List SSM ResourceDataSync resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SyncName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
