...
/**
 * Swamp extension model for Cases Field (AWS::Cases::Field).
 *
...
/** Swamp extension model for Cases Field. Registered at `@swamp/aws/cases/field`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Cases Field resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::Cases::Field", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.FieldArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
