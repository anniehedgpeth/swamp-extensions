...
/**
 * Swamp extension model for Notifications NotificationHub (AWS::Notifications::NotificationHub).
 *
...
/** Swamp extension model for Notifications NotificationHub. Registered at `@swamp/aws/notifications/notification-hub`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Notifications NotificationHub resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.Region?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
