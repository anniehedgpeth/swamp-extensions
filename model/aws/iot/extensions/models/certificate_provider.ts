...
/**
 * Swamp extension model for IoT CertificateProvider (AWS::IoT::CertificateProvider).
 *
...
/** Swamp extension model for IoT CertificateProvider. Registered at `@swamp/aws/iot/certificate-provider`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT CertificateProvider resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.CertificateProviderName?.toString() ??
