...
/**
 * Swamp extension model for IoT Authorizer (AWS::IoT::Authorizer).
 *
...
/** Swamp extension model for IoT Authorizer. Registered at `@swamp/aws/iot/authorizer`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT Authorizer resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AuthorizerName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
