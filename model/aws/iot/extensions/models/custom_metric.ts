...
/**
 * Swamp extension model for IoT CustomMetric (AWS::IoT::CustomMetric).
 *
...
/** Swamp extension model for IoT CustomMetric. Registered at `@swamp/aws/iot/custom-metric`. */
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const instanceName =
          ((result.MetricName ?? g.MetricName)?.toString() ?? "current")
    ...
    update: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.MetricName?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.MetricName?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List IoT CustomMetric resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.MetricName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
