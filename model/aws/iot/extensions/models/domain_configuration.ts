...
/**
 * Swamp extension model for IoT DomainConfiguration (AWS::IoT::DomainConfiguration).
 *
...
/** Swamp extension model for IoT DomainConfiguration. Registered at `@swamp/aws/iot/domain-configuration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT DomainConfiguration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DomainConfigurationName?.toString() ??
