...
/**
 * Swamp extension model for IoT ResourceSpecificLogging (AWS::IoT::ResourceSpecificLogging).
 *
...
/** Swamp extension model for IoT ResourceSpecificLogging. Registered at `@swamp/aws/iot/resource-specific-logging`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT ResourceSpecificLogging resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.TargetId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
