...
/**
 * Swamp extension model for IoT RoleAlias (AWS::IoT::RoleAlias).
 *
...
/** Swamp extension model for IoT RoleAlias. Registered at `@swamp/aws/iot/role-alias`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT RoleAlias resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.RoleAlias?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
