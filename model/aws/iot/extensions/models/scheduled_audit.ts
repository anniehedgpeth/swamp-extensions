...
/**
 * Swamp extension model for IoT ScheduledAudit (AWS::IoT::ScheduledAudit).
 *
...
/** Swamp extension model for IoT ScheduledAudit. Registered at `@swamp/aws/iot/scheduled-audit`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List IoT ScheduledAudit resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ScheduledAuditName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
