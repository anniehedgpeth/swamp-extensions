...
/**
 * Swamp extension model for MemoryDB User (AWS::MemoryDB::User).
 *
...
/** Swamp extension model for MemoryDB User. Registered at `@swamp/aws/memorydb/user`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List MemoryDB User resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.UserName?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
