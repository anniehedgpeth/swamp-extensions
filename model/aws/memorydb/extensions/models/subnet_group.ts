...
/**
 * Swamp extension model for MemoryDB SubnetGroup (AWS::MemoryDB::SubnetGroup).
 *
...
/** Swamp extension model for MemoryDB SubnetGroup. Registered at `@swamp/aws/memorydb/subnet-group`. */
export const model = {
  ...
  methods: {
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.SubnetGroupName?.toString() ?? args.identifier)
            .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List MemoryDB SubnetGroup resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SubnetGroupName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
