...
/**
 * Swamp extension model for NetworkManager CoreNetwork (AWS::NetworkManager::CoreNetwork).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List NetworkManager CoreNetwork resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.CoreNetworkId?.toString() ?? item.identifier)
