...
/**
 * Swamp extension model for NetworkManager TransitGatewayPeering (AWS::NetworkManager::TransitGatewayPeering).
 *
...
/** Swamp extension model for NetworkManager TransitGatewayPeering. Registered at `@swamp/aws/networkmanager/transit-gateway-peering`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List NetworkManager TransitGatewayPeering resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PeeringId?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
