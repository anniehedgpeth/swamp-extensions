...
/**
 * Swamp extension model for PCS Queue (AWS::PCS::Queue).
 *
...
const ComputeNodeGroupConfigurationSchema = z.object({
...
/** Swamp extension model for PCS Queue. Registered at `@swamp/aws/pcs/queue`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List PCS Queue resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::PCS::Queue", {
