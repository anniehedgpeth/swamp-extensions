...
/**
 * Swamp extension model for DirectoryService SimpleAD (AWS::DirectoryService::SimpleAD).
 *
...
/** Swamp extension model for DirectoryService SimpleAD. Registered at `@swamp/aws/directoryservice/simple-ad`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DirectoryService SimpleAD resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DirectoryId?.toString() ?? item.identifier)
