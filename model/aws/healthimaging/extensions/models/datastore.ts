...
/**
 * Swamp extension model for HealthImaging Datastore (AWS::HealthImaging::Datastore).
 *
...
/** Swamp extension model for HealthImaging Datastore. Registered at `@swamp/aws/healthimaging/datastore`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List HealthImaging Datastore resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DatastoreId?.toString() ?? item.identifier)
