...
/**
 * Swamp extension model for CloudFormation Publisher (AWS::CloudFormation::Publisher).
 *
...
/** Swamp extension model for CloudFormation Publisher. Registered at `@swamp/aws/cloudformation/publisher`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CloudFormation Publisher resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PublisherId?.toString() ?? item.identifier)
