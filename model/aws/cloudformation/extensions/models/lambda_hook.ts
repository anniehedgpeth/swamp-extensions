...
/**
 * Swamp extension model for CloudFormation LambdaHook (AWS::CloudFormation::LambdaHook).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CloudFormation LambdaHook resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.HookArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
