...
/**
 * Swamp extension model for CloudFormation HookTypeConfig (AWS::CloudFormation::HookTypeConfig).
 *
...
/** Swamp extension model for CloudFormation HookTypeConfig. Registered at `@swamp/aws/cloudformation/hook-type-config`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CloudFormation HookTypeConfig resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ConfigurationArn?.toString() ?? item.identifier)
