...
/**
 * Swamp extension model for CloudFormation PublicTypeVersion (AWS::CloudFormation::PublicTypeVersion).
 *
...
/** Swamp extension model for CloudFormation PublicTypeVersion. Registered at `@swamp/aws/cloudformation/public-type-version`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List CloudFormation PublicTypeVersion resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PublicTypeArn?.toString() ?? item.identifier)
