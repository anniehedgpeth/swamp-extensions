...
/**
 * Swamp extension model for CloudFormation ModuleDefaultVersion (AWS::CloudFormation::ModuleDefaultVersion).
 *
...
/** Swamp extension model for CloudFormation ModuleDefaultVersion. Registered at `@swamp/aws/cloudformation/module-default-version`. */
export const model = {
  ...
  methods: {
    ...
    get: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          ((result.Arn ?? context.globalArgs.Arn)?.toString() ??
    ...
    delete: {
      ...
      execute: async (args: { identifier: string }, context: any) => {
        ...
        const instanceName =
          (context.globalArgs.Arn?.toString() ?? args.identifier).replace(
            /[\/\\]/g,
            "_",
    ...
    sync: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildCredentials(g);
        const instanceName = (g.Arn?.toString() ?? "current").replace(
          /[\/\\]/g,
          "_",
        ).replace(/\.\./g, "_").replace(/\0/g, "");
    ...
    list: {
      description: "List CloudFormation ModuleDefaultVersion resources",
      arguments: z.object({
