...
/**
 * Swamp extension model for DevOpsGuru ResourceCollection (AWS::DevOpsGuru::ResourceCollection).
 *
...
/** Swamp extension model for DevOpsGuru ResourceCollection. Registered at `@swamp/aws/devopsguru/resource-collection`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List DevOpsGuru ResourceCollection resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.ResourceCollectionType?.toString() ??
