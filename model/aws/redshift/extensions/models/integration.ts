...
/**
 * Swamp extension model for Redshift Integration (AWS::Redshift::Integration).
 *
...
/** Swamp extension model for Redshift Integration. Registered at `@swamp/aws/redshift/integration`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Redshift Integration resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.IntegrationArn?.toString() ?? item.identifier)
