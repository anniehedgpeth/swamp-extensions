...
/**
 * Swamp extension model for Timestream Database (AWS::Timestream::Database).
 *
...
/** Swamp extension model for Timestream Database. Registered at `@swamp/aws/timestream/database`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Timestream Database resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.DatabaseName?.toString() ?? item.identifier)
              .replace(/[\/\\]/g, "_").replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
            ...item.properties,
