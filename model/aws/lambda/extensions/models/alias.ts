...
/**
 * Swamp extension model for Lambda Alias (AWS::Lambda::Alias).
 *
...
const VersionWeightSchema = z.object({
...
/** Swamp extension model for Lambda Alias. Registered at `@swamp/aws/lambda/alias`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List Lambda Alias resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        const credentials = _buildCredentials(context.globalArgs);
        const { items, nextToken } = await listResources("AWS::Lambda::Alias", {
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.AliasArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
