...
/**
 * Swamp extension model for WorkSpacesWeb IpAccessSettings (AWS::WorkSpacesWeb::IpAccessSettings).
 *
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpacesWeb IpAccessSettings resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.IpAccessSettingsArn?.toString() ??
