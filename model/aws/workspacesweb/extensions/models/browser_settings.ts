...
/**
 * Swamp extension model for WorkSpacesWeb BrowserSettings (AWS::WorkSpacesWeb::BrowserSettings).
 *
...
/** Swamp extension model for WorkSpacesWeb BrowserSettings. Registered at `@swamp/aws/workspacesweb/browser-settings`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpacesWeb BrowserSettings resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.BrowserSettingsArn?.toString() ?? item.identifier)
