...
/**
 * Swamp extension model for WorkSpacesWeb SessionLogger (AWS::WorkSpacesWeb::SessionLogger).
 *
...
const S3LogConfigurationSchema = z.object({
...
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpacesWeb SessionLogger resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.SessionLoggerArn?.toString() ?? item.identifier)
