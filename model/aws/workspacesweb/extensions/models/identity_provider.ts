...
/**
 * Swamp extension model for WorkSpacesWeb IdentityProvider (AWS::WorkSpacesWeb::IdentityProvider).
 *
...
/** Swamp extension model for WorkSpacesWeb IdentityProvider. Registered at `@swamp/aws/workspacesweb/identity-provider`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpacesWeb IdentityProvider resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.IdentityProviderArn?.toString() ??
