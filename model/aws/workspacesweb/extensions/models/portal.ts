...
/**
 * Swamp extension model for WorkSpacesWeb Portal (AWS::WorkSpacesWeb::Portal).
 *
...
/** Swamp extension model for WorkSpacesWeb Portal. Registered at `@swamp/aws/workspacesweb/portal`. */
export const model = {
  ...
  methods: {
    ...
    list: {
      description: "List WorkSpacesWeb Portal resources",
      arguments: z.object({
      ...
      execute: async (
        ...
        for (let i = 0; i < items.length; i++) {
          const item = items[i];
          const instanceName =
            (item.properties?.PortalArn?.toString() ?? item.identifier).replace(
              /[\/\\]/g,
              "_",
            ).replace(/\.\./g, "_").replace(/\0/g, "");
          const handle = await context.writeResource("state", instanceName, {
