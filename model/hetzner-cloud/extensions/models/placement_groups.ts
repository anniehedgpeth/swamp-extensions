...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Find an existing placement group by name and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        ...
        if (matches.length === 0) {
          throw new Error(
            `No placement group found matching name=${expectedName}`,
          );
        }
        if (matches.length > 1) {
          throw new Error(
            `Multiple placement groups found matching name=${expectedName} (found ${matches.length}). Use adopt with a specific ID instead.`,
          );
        }
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.number().int().describe("The ID of the placement group to adopt"),
