  ...
  methods: {
    ...
    sync: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const existing = JSON.parse(new TextDecoder().decode(content));
        const result = await tryRead("/servers", existing.id, g.token) as
          | ResourceData
          | null;
        if (result) {
          const handle = await context.writeResource(
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (matches.length > 1) {
          throw new Error(
            `Multiple servers found matching name=${expectedName} (found ${matches.length}). Use adopt with a specific ID instead.`,
          );
        }
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.number().int().describe("The ID of the server to adopt"),
    ...
    change_protection: {
      description: "Change delete/rebuild protection for the server",
      arguments: z.object({
        delete: z.boolean().describe(
          "If true, prevents the server from being deleted",
        ).optional(),
        rebuild: z.boolean().describe(
          "If true, prevents the server from being rebuilt",
        ).optional(),
      }),
      execute: async (
        args: { delete?: boolean; rebuild?: boolean },
        context: any,
      ) => {
        const g = context.globalArgs;
        const instanceName = (g.name?.toString() ?? "current").replace(
