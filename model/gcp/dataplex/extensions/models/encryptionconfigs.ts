...
/**
 * Swamp extension model for Google Cloud Dataplex EncryptionConfigs.
 *
...
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["encryptionConfigId"] !== undefined) {
          params["encryptionConfigId"] = String(g["encryptionConfigId"]);
        }
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific encryptionConfigs by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List encryptionConfigs resources",
      arguments: z.object({
