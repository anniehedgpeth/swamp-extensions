...
Google Cloud apikeys resources.
...
Use `swamp model type describe @swamp/gcp/apikeys/keys` to see the full list of
...
```bash
# Create a new keys model
swamp model create @swamp/gcp/apikeys/keys my-keys

# Edit the model to configure its properties
swamp model edit my-keys

# Create the resource in GCP
swamp model method run my-keys create

# Sync current state from GCP
swamp model method run my-keys sync
```
