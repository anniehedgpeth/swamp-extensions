...
Use `swamp model type describe @swamp/gcp/chromewebstore/items` to see the full
...
```bash
# Create a new items model
swamp model create @swamp/gcp/chromewebstore/items my-items

# Edit the model to configure its properties
swamp model edit my-items

# Create the resource in GCP
swamp model method run my-items create

# Sync current state from GCP
swamp model method run my-items sync
```
