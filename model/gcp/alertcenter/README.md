...
Google Cloud alertcenter resources.
...
Use `swamp model type describe @swamp/gcp/alertcenter/alerts_feedback` to see
...
```bash
# Create a new alerts_feedback model
swamp model create @swamp/gcp/alertcenter/alerts_feedback my-alerts_feedback

# Edit the model to configure its properties
swamp model edit my-alerts_feedback

# Create the resource in GCP
swamp model method run my-alerts_feedback create

# Sync current state from GCP
swamp model method run my-alerts_feedback sync
```
