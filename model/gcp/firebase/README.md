...
Google Cloud firebase resources.
...
Use `swamp model type describe @swamp/gcp/firebase/androidapps_sha` to see the
...
```bash
# Create a new androidapps_sha model
swamp model create @swamp/gcp/firebase/androidapps_sha my-androidapps_sha

# Edit the model to configure its properties
swamp model edit my-androidapps_sha

# Create the resource in GCP
swamp model method run my-androidapps_sha create

# Sync current state from GCP
swamp model method run my-androidapps_sha sync
```
