...
Google Cloud bigquerydatatransfer resources.
...
Use `swamp model type describe @swamp/gcp/bigquerydatatransfer/datasources` to
...
```bash
# Create a new datasources model
swamp model create @swamp/gcp/bigquerydatatransfer/datasources my-datasources

# Edit the model to configure its properties
swamp model edit my-datasources

# Create the resource in GCP
swamp model method run my-datasources create

# Sync current state from GCP
swamp model method run my-datasources sync
```
