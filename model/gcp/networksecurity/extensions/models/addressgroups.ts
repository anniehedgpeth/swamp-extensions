...
/**
 * Swamp extension model for Google Cloud Network Security AddressGroups.
 *
    ...
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["addressGroupId"] !== undefined) {
          params["addressGroupId"] = String(g["addressGroupId"]);
        }
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific addressGroups by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
