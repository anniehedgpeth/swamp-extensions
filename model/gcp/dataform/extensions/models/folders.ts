...
// Auto-generated extension model for @swamp/gcp/dataform/folders
// Do not edit manually. Re-generate with: deno task generate:gcp

// deno-lint-ignore-file no-explicit-any

/**
 * Swamp extension model for Google Cloud Dataform Folders.
 *
...
function buildResourceName(parent: string, shortName: string): string {
  return `${parent}/folders/${shortName}`;
}
...
const GlobalArgsSchema = z.object({
  ...
  displayName: z.string().describe("Required. The Folder's user-friendly name.")
    .optional(),
  name: z.string().describe("Identifier. The Folder's name.").optional(),
...
export const model = {
  ...
  resources: {
    state: {
      description:
        "Represents a Dataform Folder. This is a resource that is used to organize Fil...",
      schema: StateSchema,
      lifetime: "infinite",
  ...
  methods: {
    ...
    query_folder_contents: {
      description: "query folder contents",
      arguments: z.object({}),
      execute: async (_args: Record<string, unknown>, context: any) => {
        const g = context.globalArgs;
        const credentials = _buildGcpCredentials(g);
        ...
        const existing = JSON.parse(new TextDecoder().decode(content));
        params["folder"] = existing["name"]?.toString() ??
          g["name"]?.toString() ?? "";
        const result = await createResource(
          BASE_URL,
          {
            ...
            "httpMethod": "GET",
            "parameterOrder": ["folder"],
            "parameters": {
              "filter": { "location": "query" },
              "folder": { "location": "path", "required": true },
