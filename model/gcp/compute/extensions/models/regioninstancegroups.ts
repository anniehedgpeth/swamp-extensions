...
/**
 * Swamp extension model for Google Cloud Compute Engine RegionInstanceGroups.
 *
...
const GlobalArgsSchema = z.object({
  ...
  region: z.string().describe("Name of the region scoping this request."),
});
...
export const model = {
  ...
  methods: {
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific regionInstanceGroups by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List regionInstanceGroups resources",
      arguments: z.object({
      ...
      execute: async (args: Record<string, unknown>, context: any) => {
        ...
        const result = await createResource(
          BASE_URL,
          {
            ...
            "path":
              "projects/{project}/regions/{region}/instanceGroups/{resource}/testIamPermissions",
            "httpMethod": "POST",
