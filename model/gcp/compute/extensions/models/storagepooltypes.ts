...
/**
 * Swamp extension model for Google Cloud Compute Engine StoragePoolTypes.
 *
...
/** Swamp extension model for Google Cloud Compute Engine StoragePoolTypes. Registered at `@swamp/gcp/compute/storagepooltypes`. */
export const model = {
  ...
  methods: {
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific storagePoolTypes by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List storagePoolTypes resources",
      arguments: z.object({
