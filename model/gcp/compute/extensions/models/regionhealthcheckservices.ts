...
/**
 * Swamp extension model for Google Cloud Compute Engine RegionHealthCheckServices.
 *
...
/** Swamp extension model for Google Cloud Compute Engine RegionHealthCheckServices. Registered at `@swamp/gcp/compute/regionhealthcheckservices`. */
export const model = {
  ...
  methods: {
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific regionHealthCheckServices by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List regionHealthCheckServices resources",
      arguments: z.object({
      ...
      execute: async (args: Record<string, unknown>, context: any) => {
        ...
        const result = await createResource(
          BASE_URL,
          {
            ...
            "path":
              "projects/{project}/regions/{region}/healthCheckServices/{resource}/testIamPermissions",
            "httpMethod": "POST",
