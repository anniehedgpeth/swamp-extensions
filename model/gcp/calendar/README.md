...
Google Cloud calendar resources.
...
Use `swamp model type describe @swamp/gcp/calendar/acl` to see the full list of
...
```bash
# Create a new acl model
swamp model create @swamp/gcp/calendar/acl my-acl

# Edit the model to configure its properties
swamp model edit my-acl

# Create the resource in GCP
swamp model method run my-acl create

# Sync current state from GCP
swamp model method run my-acl sync
```
