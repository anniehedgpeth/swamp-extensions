...
const GlobalArgsSchema = z.object({
  ...
  accountId: z.string().describe("Account Id for the upload to retrieve."),
  webPropertyId: z.string().describe(
    "Web property Id for the upload to retrieve.",
  ),
  customDataSourceId: z.string().describe(
...
export const model = {
  ...
  upgrades: [
    ...
    },
    {
      ...
      description:
        "Added: accessToken, credentialsJson, project, scopes, accountId, webPropertyId, customDataSourceId",
      upgradeAttributes: (old: Record<string, unknown>) => old,
    },
    {
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific uploads by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
