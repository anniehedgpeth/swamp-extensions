...
Google Cloud firestore resources.
...
Use `swamp model type describe @swamp/gcp/firestore/backups` to see the full
...
```bash
# Create a new backups model
swamp model create @swamp/gcp/firestore/backups my-backups

# Edit the model to configure its properties
swamp model edit my-backups

# Create the resource in GCP
swamp model method run my-backups create

# Sync current state from GCP
swamp model method run my-backups sync
```
