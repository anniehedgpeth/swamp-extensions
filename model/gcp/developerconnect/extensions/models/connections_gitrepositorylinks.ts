...
/**
 * Swamp extension model for Google Cloud Developer Connect Connections.GitRepositoryLinks.
 *
...
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["gitRepositoryLinkId"] !== undefined) {
          params["gitRepositoryLinkId"] = String(g["gitRepositoryLinkId"]);
        }
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific gitRepositoryLinks by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List gitRepositoryLinks resources",
      arguments: z.object({
