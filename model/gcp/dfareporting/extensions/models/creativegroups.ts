...
/**
 * Swamp extension model for Google Cloud Campaign Manager 360 CreativeGroups.
 *
...
/** Swamp extension model for Google Cloud Campaign Manager 360 CreativeGroups. Registered at `@swamp/gcp/dfareporting/creativegroups`. */
export const model = {
  ...
  methods: {
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific creativeGroups by id (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List creativeGroups resources",
      arguments: z.object({
        advertiserIds: z.string().describe(
          "Select only creative groups that belong to these advertisers.",
        ).optional(),
        groupNumber: z.number().describe(
          "Select only creative groups that belong to this subgroup.",
        ).optional(),
        ids: z.string().describe("Select only creative groups with these IDs.")
          .optional(),
      ...
      execute: async (args: Record<string, unknown>, context: any) => {
        ...
        if (args["groupNumber"] !== undefined) {
          params["groupNumber"] = String(args["groupNumber"]);
        }
