...
/**
 * Swamp extension model for Google Cloud Firebase App Hosting Backends.
 *
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["backendId"] !== undefined) {
          params["backendId"] = String(g["backendId"]);
        }
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific backends by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List backends resources",
      arguments: z.object({
