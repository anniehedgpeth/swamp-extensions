...
Google Cloud vmwareengine resources.
...
Use `swamp model type describe @swamp/gcp/vmwareengine/announcements` to see the
...
```bash
# Create a new announcements model
swamp model create @swamp/gcp/vmwareengine/announcements my-announcements

# Edit the model to configure its properties
swamp model edit my-announcements

# Create the resource in GCP
swamp model method run my-announcements create

# Sync current state from GCP
swamp model method run my-announcements sync
```
