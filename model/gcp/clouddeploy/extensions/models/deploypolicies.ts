...
/**
 * Swamp extension model for Google Cloud Deploy DeployPolicies.
 *
...
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["deployPolicyId"] !== undefined) {
          params["deployPolicyId"] = String(g["deployPolicyId"]);
        }
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific deployPolicies by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List deployPolicies resources",
      arguments: z.object({
