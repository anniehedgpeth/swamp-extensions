...
Google Cloud drivelabels resources.
...
Use `swamp model type describe @swamp/gcp/drivelabels/labels_locks` to see the
...
```bash
# Create a new labels_locks model
swamp model create @swamp/gcp/drivelabels/labels_locks my-labels_locks

# Edit the model to configure its properties
swamp model edit my-labels_locks

# Create the resource in GCP
swamp model method run my-labels_locks create

# Sync current state from GCP
swamp model method run my-labels_locks sync
```
