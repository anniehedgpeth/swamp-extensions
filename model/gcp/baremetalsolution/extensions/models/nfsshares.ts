...
/**
 * Swamp extension model for Google Cloud Bare Metal Solution NfsShares.
 *
 * An NFS share.
 *
...
/** Swamp extension model for Google Cloud Bare Metal Solution NfsShares. Registered at `@swamp/gcp/baremetalsolution/nfsshares`. */
export const model = {
  ...
  methods: {
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific nfsShares by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List nfsShares resources",
      arguments: z.object({
