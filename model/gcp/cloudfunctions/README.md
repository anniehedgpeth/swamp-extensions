...
Google Cloud cloudfunctions resources.
...
Use `swamp model type describe @swamp/gcp/cloudfunctions/functions` to see the
...
```bash
# Create a new functions model
swamp model create @swamp/gcp/cloudfunctions/functions my-functions

# Edit the model to configure its properties
swamp model edit my-functions

# Create the resource in GCP
swamp model method run my-functions create

# Sync current state from GCP
swamp model method run my-functions sync
```
