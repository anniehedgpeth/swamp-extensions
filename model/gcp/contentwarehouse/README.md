...
Google Cloud contentwarehouse resources.
...
Use
`swamp model type describe @swamp/gcp/contentwarehouse/documents_referenceid` to
...
```bash
# Create a new documents_referenceid model
swamp model create @swamp/gcp/contentwarehouse/documents_referenceid my-documents_referenceid

# Edit the model to configure its properties
swamp model edit my-documents_referenceid

# Create the resource in GCP
swamp model method run my-documents_referenceid create

# Sync current state from GCP
swamp model method run my-documents_referenceid sync
```
