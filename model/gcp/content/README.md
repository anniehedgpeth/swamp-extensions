...
Google Cloud content resources.
...
Use `swamp model type describe @swamp/gcp/content/accounts_labels` to see the
...
```bash
# Create a new accounts_labels model
swamp model create @swamp/gcp/content/accounts_labels my-accounts_labels

# Edit the model to configure its properties
swamp model edit my-accounts_labels

# Create the resource in GCP
swamp model method run my-accounts_labels create

# Sync current state from GCP
swamp model method run my-accounts_labels sync
```
