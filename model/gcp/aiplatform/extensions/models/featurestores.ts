...
/**
 * Swamp extension model for Google Cloud Vertex AI Featurestores.
 *
...
/** Swamp extension model for Google Cloud Vertex AI Featurestores. Registered at `@swamp/gcp/aiplatform/featurestores`. */
export const model = {
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["featurestoreId"] !== undefined) {
          params["featurestoreId"] = String(g["featurestoreId"]);
        }
        if (g["name"] !== undefined) {
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific featurestores by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
    ...
    list: {
      description: "List featurestores resources",
      arguments: z.object({
