...
/**
 * Swamp extension model for Google Cloud Agent Platform Tensorboards.Experiments.Runs.TimeSeries.
 *
    ...
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g["tensorboardTimeSeriesId"] !== undefined) {
          params["tensorboardTimeSeriesId"] = String(
            g["tensorboardTimeSeriesId"],
          );
        }
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific timeSeries by name (e.g. one discovered by list)",
        ).optional(),
      }),
      execute: async (args: { identifier?: string }, context: any) => {
        const g = context.globalArgs;
