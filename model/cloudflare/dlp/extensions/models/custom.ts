...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Custom by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/dlp/profiles/custom";
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No custom found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Custom by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Custom to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/dlp/profiles/custom";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Custom by id (e.g. one discovered by list)",
        ).optional(),
      }),
