...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Data Classes by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/dlp/data_classes";
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No data classes found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Data Classes by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Data Classes to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/dlp/data_classes";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Data Classes by id (e.g. one discovered by list)",
        ).optional(),
      }),
