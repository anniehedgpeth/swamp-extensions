...
// Auto-generated extension model for @swamp/cloudflare/vectorize/indexes
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        const endpoint = "/accounts/" + g.account_id + "/vectorize/indexes";
    ...
    lookup: {
      description:
        "Look up an existing Indexes by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/vectorize/indexes";
        ...
        if (g.config !== undefined) filters.push(["config", String(g.config)]);
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No indexes found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Indexes by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Indexes to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/vectorize/indexes";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Indexes by name (e.g. one discovered by list)",
        ).optional(),
      }),
