...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Site Info by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/rum/site_info";
        const filters: [string, string][] = [];
        if (g.auto_install !== undefined) {
          filters.push(["auto_install", String(g.auto_install)]);
        }
        if (g.enabled !== undefined) {
          filters.push(["enabled", String(g.enabled)]);
        }
        if (g.host !== undefined) filters.push(["host", String(g.host)]);
        if (g.lite !== undefined) filters.push(["lite", String(g.lite)]);
        if (g.zone_tag !== undefined) {
          filters.push(["zone_tag", String(g.zone_tag)]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No site info found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description:
        "Import an existing Site Info by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Site Info to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/rum/site_info";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Site Info by id (e.g. one discovered by list)",
        ).optional(),
      }),
