...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Ip-profiles by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/devices/ip-profiles";
        ...
        if (g.subnet_id !== undefined) {
          filters.push(["subnet_id", String(g.subnet_id)]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No ip-profiles found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Ip-profiles by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Ip-profiles to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/devices/ip-profiles";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Ip-profiles by id (e.g. one discovered by list)",
        ).optional(),
      }),
