...
// Auto-generated extension model for @swamp/cloudflare/load-balancing/preview
// Do not edit manually. Re-generate with: deno task generate:cloudflare

// deno-lint-ignore-file no-explicit-any

/**
 * Swamp extension model for a Cloudflare Preview.
 *
...
export const model = {
  ...
  resources: {
    state: {
      description: "Preview resource state",
  ...
  methods: {
    ...
    get: {
      description: "Get a Preview",
      arguments: z.object({ id: z.string().describe("The ID of the Preview") }),
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.consecutive_fails !== undefined) {
          filters.push(["consecutive_fails", String(g.consecutive_fails)]);
        }
        if (g.consecutive_successes !== undefined) {
          filters.push([
            "consecutive_successes",
            String(g.consecutive_successes),
          ]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No preview found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Preview by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Preview to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id + "/healthchecks/preview";
    ...
    delete: {
      description: "Delete the Preview",
      arguments: z.object({ id: z.string().describe("The ID of the Preview") }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id + "/healthchecks/preview";
    ...
    sync: {
      description: "Sync Preview state from Cloudflare",
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Preview by id (e.g. one discovered by list)",
        ).optional(),
      }),
