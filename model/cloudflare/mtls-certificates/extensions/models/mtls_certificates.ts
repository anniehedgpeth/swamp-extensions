...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Mtls Certificates by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/mtls_certificates";
        const filters: [string, string][] = [];
        if (g.ca !== undefined) filters.push(["ca", String(g.ca)]);
        if (g.certificates !== undefined) {
          filters.push(["certificates", String(g.certificates)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No mtls certificates found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Mtls Certificates by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Mtls Certificates to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/mtls_certificates";
