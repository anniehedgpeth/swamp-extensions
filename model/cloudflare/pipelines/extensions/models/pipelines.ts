...
// Auto-generated extension model for @swamp/cloudflare/pipelines/pipelines
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.sql !== undefined) body.sql = g.sql;
    ...
    lookup: {
      description:
        "Look up an existing Pipelines by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
        ...
        if (g.sql !== undefined) filters.push(["sql", String(g.sql)]);
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No pipelines found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description:
        "Import an existing Pipelines by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Pipelines to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Pipelines by id (e.g. one discovered by list)",
        ).optional(),
      }),
