  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.custom_html !== undefined) {
          filters.push(["custom_html", String(g.custom_html)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No custom pages found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.string().describe("The ID of the Custom Pages to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/access/custom_pages";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Custom Pages by id (e.g. one discovered by list)",
        ).optional(),
      }),
