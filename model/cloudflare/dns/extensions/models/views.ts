  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.created_time !== undefined) {
          filters.push(["created_time", String(g.created_time)]);
        }
        if (g.modified_time !== undefined) {
          filters.push(["modified_time", String(g.modified_time)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No views found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Views by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Views to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/dns_settings/views";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Views by id (e.g. one discovered by list)",
        ).optional(),
      }),
