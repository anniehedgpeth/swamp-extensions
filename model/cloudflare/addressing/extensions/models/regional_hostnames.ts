...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Regional Hostnames by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id +
        ...
        if (g.routing !== undefined) {
          filters.push(["routing", String(g.routing)]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No regional hostnames found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Regional Hostnames by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Regional Hostnames to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Regional Hostnames by id (e.g. one discovered by list)",
        ).optional(),
      }),
