...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Address Maps by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
        ...
        if (g.default_sni !== undefined) {
          filters.push(["default_sni", String(g.default_sni)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No address maps found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Address Maps by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Address Maps to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Address Maps by id (e.g. one discovered by list)",
        ).optional(),
      }),
