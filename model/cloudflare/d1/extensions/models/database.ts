...
export const model = {
  ...
  methods: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.primary_location_hint !== undefined) {
          filters.push([
            ...
            String(g.primary_location_hint),
          ]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No database found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.string().describe("The ID of the Database to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/d1/database";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Database by id (e.g. one discovered by list)",
        ).optional(),
      }),
