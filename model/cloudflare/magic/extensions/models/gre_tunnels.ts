...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Gre Tunnels by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/magic/gre_tunnels";
        ...
        if (g.cloudflare_gre_endpoint !== undefined) {
          filters.push([
            ...
            String(g.cloudflare_gre_endpoint),
          ]);
        }
        if (g.customer_gre_endpoint !== undefined) {
          filters.push([
            "customer_gre_endpoint",
            String(g.customer_gre_endpoint),
          ]);
        }
        ...
        if (g.mtu !== undefined) filters.push(["mtu", String(g.mtu)]);
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No gre tunnels found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Gre Tunnels by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Gre Tunnels to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/magic/gre_tunnels";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Gre Tunnels by id (e.g. one discovered by list)",
        ).optional(),
      }),
