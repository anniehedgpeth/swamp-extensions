...
// Auto-generated extension model for @swamp/cloudflare/magic/prefixes
  ...
  methods: {
    create: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.excluded !== undefined) body.excluded = g.excluded;
    ...
    lookup: {
      description:
        "Look up an existing Prefixes by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
        ...
        if (g.excluded !== undefined) {
          filters.push(["excluded", String(g.excluded)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No prefixes found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description:
        "Import an existing Prefixes by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Prefixes to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Prefixes by id (e.g. one discovered by list)",
        ).optional(),
      }),
