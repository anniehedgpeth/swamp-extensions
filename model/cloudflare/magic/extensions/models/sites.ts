...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Sites by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/magic/sites";
        const filters: [string, string][] = [];
        if (g.connector_id !== undefined) {
          filters.push(["connector_id", String(g.connector_id)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No sites found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Sites by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Sites to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/magic/sites";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Sites by id (e.g. one discovered by list)",
        ).optional(),
      }),
