...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Variants by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/images/v1/variants";
        ...
        if (g.neverRequireSignedURLs !== undefined) {
          filters.push([
            ...
            String(g.neverRequireSignedURLs),
          ]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No variants found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description:
        "Import an existing Variants by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Variants to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/images/v1/variants";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Variants by id (e.g. one discovered by list)",
        ).optional(),
      }),
