...
Use `swamp model type describe @swamp/cloudflare/agent-memory/namespaces` to see
...
```bash
# Create a new namespaces model
swamp model create @swamp/cloudflare/agent-memory/namespaces my-namespaces

# Edit the model to configure its properties
swamp model edit my-namespaces

# Create the resource in Cloudflare
swamp model method run my-namespaces create

# Sync current state from Cloudflare
swamp model method run my-namespaces sync
```
