...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Origin Cloud Regions by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id + "/cache/origin_cloud_regions";
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No origin cloud regions found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Origin Cloud Regions by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Origin Cloud Regions to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id + "/cache/origin_cloud_regions";
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Origin Cloud Regions by id (e.g. one discovered by list)",
        ).optional(),
      }),
