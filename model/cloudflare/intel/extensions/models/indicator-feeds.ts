...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Indicator-feeds by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/intel/indicator-feeds";
        ...
        if (g.is_attributable !== undefined) {
          filters.push(["is_attributable", String(g.is_attributable)]);
        }
        if (g.is_downloadable !== undefined) {
          filters.push(["is_downloadable", String(g.is_downloadable)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No indicator-feeds found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      description:
        "Import an existing Indicator-feeds by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Indicator-feeds to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/intel/indicator-feeds";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Indicator-feeds by id (e.g. one discovered by list)",
        ).optional(),
      }),
