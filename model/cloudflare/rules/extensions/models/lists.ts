...
// Auto-generated extension model for @swamp/cloudflare/rules/lists
...
const InputsSchema = z.object({
  ...
  kind: z.enum(["ip", "redirect", "hostname", "asn"]).optional(),
  name: z.string().max(50).regex(new RegExp("^[a-zA-Z0-9_]+$")).optional(),
...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No lists found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Lists by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Lists to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/rules/lists";
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Lists by id (e.g. one discovered by list)",
        ).optional(),
      }),
