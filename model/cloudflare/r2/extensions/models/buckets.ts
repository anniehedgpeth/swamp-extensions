...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Buckets by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/r2/buckets";
        ...
        if (g.locationHint !== undefined) {
          filters.push(["locationHint", String(g.locationHint)]);
        }
        if (g.name !== undefined) filters.push(["name", String(g.name)]);
        if (g.storageClass !== undefined) {
          filters.push(["storageClass", String(g.storageClass)]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No buckets found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Buckets by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Buckets to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id + "/r2/buckets";
