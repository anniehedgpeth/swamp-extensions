...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No custom trust store found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.string().describe("The ID of the Custom Trust Store to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/zones/" + g.zone_id + "/acm/custom_trust_store";
    ...
    sync: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Custom Trust Store by id (e.g. one discovered by list)",
        ).optional(),
      }),
