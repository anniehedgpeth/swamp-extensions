...
Use `swamp model type describe @swamp/cloudflare/ssl/custom-trust-store` to see
...
```bash
# Create a new custom-trust-store model
swamp model create @swamp/cloudflare/ssl/custom-trust-store my-custom-trust-store

# Edit the model to configure its properties
swamp model edit my-custom-trust-store

# Create the resource in Cloudflare
swamp model method run my-custom-trust-store create

# Sync current state from Cloudflare
swamp model method run my-custom-trust-store sync
```
