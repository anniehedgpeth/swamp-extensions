...
export const model = {
  ...
  methods: {
    ...
    lookup: {
      description:
        "Look up an existing Warp by matching global argument values and import it into state",
      arguments: z.object({}),
      execute: async (_args: Record<string, never>, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
        ...
        if (g.network !== undefined) {
          filters.push(["network", String(g.network)]);
        }
        if (filters.length === 0) {
          throw new Error(
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No warp found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Warp by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Warp to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Warp by id (e.g. one discovered by list)",
        ).optional(),
      }),
