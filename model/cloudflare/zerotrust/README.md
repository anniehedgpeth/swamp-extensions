...
Use `swamp model type describe @swamp/cloudflare/zerotrust/hostname` to see the
...
```bash
# Create a new hostname model
swamp model create @swamp/cloudflare/zerotrust/hostname my-hostname

# Edit the model to configure its properties
swamp model edit my-hostname

# Create the resource in Cloudflare
swamp model method run my-hostname create

# Sync current state from Cloudflare
swamp model method run my-hostname sync
```
