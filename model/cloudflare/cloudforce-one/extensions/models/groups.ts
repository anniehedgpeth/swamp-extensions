...
// Auto-generated extension model for @swamp/cloudflare/cloudforce-one/groups
...
/** Swamp extension model for Cloudflare Groups. Registered at `@swamp/cloudflare/cloudforce-one/groups`. */
export const model = {
  type: "@swamp/cloudflare/cloudforce-one/groups",
  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (matches.length === 0) {
          ...
          throw new Error(`No groups found matching filters: ${filterDesc}`);
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
            `${k}=${JSON.stringify(v)}`
    ...
    adopt: {
      description: "Import an existing Groups by ID into state for management",
      arguments: z.object({
        id: z.string().describe("The ID of the Groups to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Groups by id (e.g. one discovered by list)",
        ).optional(),
      }),
