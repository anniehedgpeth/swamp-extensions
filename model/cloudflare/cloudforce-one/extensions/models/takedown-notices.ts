  ...
  methods: {
    ...
    lookup: {
      ...
      execute: async (_args: Record<string, never>, context: any) => {
        ...
        if (g.matchId !== undefined) {
          filters.push(["matchId", String(g.matchId)]);
        }
        if (g.matchType !== undefined) {
          filters.push(["matchType", String(g.matchType)]);
        }
        ...
        if (matches.length === 0) {
          ...
          throw new Error(
            `No takedown-notices found matching filters: ${filterDesc}`,
          );
        }
        if (matches.length > 1) {
          const filterDesc = filters.map(([k, v]) =>
    ...
    adopt: {
      ...
      arguments: z.object({
        id: z.string().describe("The ID of the Takedown-notices to import"),
      }),
      execute: async (args: { id: string }, context: any) => {
        const g = context.globalArgs;
        const endpoint = "/accounts/" + g.account_id +
    ...
    update: {
      ...
      arguments: z.object({
        identifier: z.string().describe(
          "Target a specific Takedown-notices by id (e.g. one discovered by list)",
        ).optional(),
      }),
